...
Enemy4::Enemy4()
{
	...
	m_Attack_A->AddContinueFrame(L"Painting/Enemy4/attack-A", 1, 6);

	m_Attack_B = new Animation();
	m_Attack_B->SetParent(this);
	m_Attack_B->Init(5, true);
	m_Attack_B->AddContinueFrame(L"Painting/Enemy4/attack-B", 1, 10);

	m_Dead = new Animation();
	m_Dead->SetParent(this);
	m_Dead->Init(7, true);
	m_Dead->AddContinueFrame(L"Painting/Enemy4/dead-", 1, 4);

	m_Hit = new Animation();
	m_Hit->SetParent(this);
	m_Hit->Init(7, true);
	m_Hit->AddContinueFrame(L"Painting/Enemy4/hit-", 1, 3);

	m_Idle = new Animation();
	m_Idle->SetParent(this);
	m_Idle->Init(10, true);
	m_Idle->AddContinueFrame(L"Painting/Enemy4/idle-", 1, 2);

	m_Jump = new Animation();
	m_Jump->SetParent(this);
	m_Jump->Init(3, true);
	m_Jump->AddContinueFrame(L"Painting/Enemy4/jump-", 1, 5);

	m_Walk = new Animation();
	m_Walk->SetParent(this);
	m_Walk->Init(3, true);
	m_Walk->AddContinueFrame(L"Painting/Enemy4/walk-", 1, 6);
	...
	m_Spearman = m_Idle;
	m_Spearman->SetParent(this);
	m_EnemyStatus4 = EnemyStatus::IDLE;
...
void Enemy4::Attack()
{
	if (m_EnemyStatus4 != EnemyStatus::ATTACK_A && m_EnemyStatus4 != EnemyStatus::JUMP)
	{
		m_EnemyStatus4 = EnemyStatus::ATTACK_A;
	}

	if (m_EnemyStatus4 == EnemyStatus::ATTACK_A)
	{
		m_Spearman = m_Attack_A;

		if (m_Spearman->m_CurrentFrame == 6 - 1)
		{
			m_AttackLate = 0.3f;
...
void Enemy4::Jump()
{
	if (m_EnemyStatus4 != EnemyStatus::JUMP && m_JumpLate <= 0.f)
	{
		m_JumpTime = 0;
		Pos = m_Position;
		m_EnemyStatus4 = EnemyStatus::JUMP;
		//ObjMgr->AddObject(new EffectMgr(L"Painting/Enemy1/attack-A", 1, 7, 5, m_Position), "Effect");
	}
	if (m_EnemyStatus4 == EnemyStatus::JUMP)
	{
		static float minus;
		m_Spearman = m_Jump;
...
void Enemy4::SetLookingDirection()
{
...
void Enemy4::Update(float deltaTime, float Time)
{
	m_isGround = false;
	m_RightCol = false;
	m_LeftCol = false;
	...
	//if()
	if (m_EnemyStatus4 != EnemyStatus::ATTACK_A && m_EnemyStatus4 != EnemyStatus::ATTACK_B)
	{
		Move();
		Jump();
	}
	...
	m_Spearman->Update(deltaTime, Time);
...
void Enemy4::OnCollision(Object* other)
{
